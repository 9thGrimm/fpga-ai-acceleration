...
def reconstruct_layer2_vector(data: np.ndarray, num_filters: int = 4) -> np.ndarray:
    out = np.full((num_filters,), fill_value=np.nan, dtype=np.float64)

    if data.shape[0] == 0:
        return out

    for idx, (filt, val) in enumerate(data):
        filt = int(filt)
        val = int(val)

        if not (0 <= filt < num_filters):
            raise ValueError(f"Invalid Layer-2 filter index at line {idx}: {filt}")
...
def compare_layer2_vector(name: str, rtl_vec: np.ndarray, py_vec: np.ndarray) -> None:
    print("\n" + "=" * 80)
    print(name)
    print("=" * 80)
    ...
    print("\nPython Layer-2 Output:")
    print(py_vec)

    mask = ~np.isnan(rtl_vec)
    ...
    if np.count_nonzero(mask) != py_vec.size:
        print("FAIL")
        print("Not all Layer-2 RTL outputs were populated.")
        return

    rtl_int = rtl_vec.astype(np.int32)
    py_int = py_vec.astype(np.int32)

    if np.array_equal(rtl_int, py_int):
        print("PASS")
    else:
        print("FAIL")
...
def compare_classifier_output(
    rtl_data: np.ndarray,
    py_pred_class: int,
    py_pred_max: int
) -> None:
    print("\n" + "=" * 80)
    print("FULL PIPELINE CLASSIFIER OUTPUT CHECK")
    print("=" * 80)

    if rtl_data.shape[0] == 0:
        print("[INFO] No RTL classifier output found.")
        return
    ...
    print(f"class={py_pred_class}, max_value={py_pred_max}")

    if (rtl_pred_class == py_pred_class) and (rtl_pred_max == py_pred_max):
        print("\nFULL PIPELINE CLASSIFIER COMPARISON: PASS")
    else:
        print("\nFULL PIPELINE CLASSIFIER COMPARISON: FAIL")
        print(f"Class diff: RTL={rtl_pred_class}, Python={py_pred_class}")
        print(f"Max diff:   RTL={rtl_pred_max}, Python={py_pred_max}")
...
# ------------------------------------------------------------
# Layer-2 Golden Model: 4 input channels × 3x3 → 4 output filters
# ------------------------------------------------------------

def get_layer2_weights() -> np.ndarray:
    """
    weights shape:
      [out_filters=4, in_channels=4, kernel_rows=3, kernel_cols=3]

    Small deterministic weights for RTL-friendly debugging.
    Values are intentionally simple: -1, 0, +1.
    """
    w = np.zeros((4, 4, 3, 3), dtype=np.int32)

    # Filter 0: emphasize channel 0 + mild channel 2 contribution
    w[0, 0] = np.array([
        [ 1,  0, -1],
        [ 1,  0, -1],
        [ 1,  0, -1]
    ...
    # Filter 2: combine all channels lightly
    for ch in range(4):
        w[2, ch] = np.array([
            [1, 0, 1],
            [0, 1, 0],
    ...
    # Filter 3: signed contrast across channels
    w[3, 0] = np.array([
        [ 1,  1,  1],
        [ 1,  1,  1],
        [ 1,  1,  1]
...
def layer2_conv_3x3_multichannel(
    fmap: np.ndarray,
    weights: np.ndarray,
    ...
    """
    fmap shape:
      [in_channels=4, rows=3, cols=3]

    weights shape:
      [out_filters=4, in_channels=4, 3, 3]

    output shape:
      [out_filters=4]
    """
    out_filters = weights.shape[0]
    in_channels = weights.shape[1]

    out = np.zeros((out_filters,), dtype=np.int32)

    for oc in range(out_filters):
        acc = 0

        for ic in range(in_channels):
            acc += int(np.sum(fmap[ic] * weights[oc, ic]))
...
def print_layer2_debug(fmap: np.ndarray, weights: np.ndarray, out_raw: np.ndarray, out_relu: np.ndarray, out_quant: np.ndarray) -> None:
    print("\n" + "=" * 80)
    print("LAYER-2 GOLDEN MODEL DEBUG")
    print("=" * 80)

    print("\nLayer-2 Input Feature Map: shape =", fmap.shape)
    for ch in range(fmap.shape[0]):
        print(f"\nInput Channel {ch}:")
        print(fmap[ch])
    ...
    for oc in range(weights.shape[0]):
        ...
        for ic in range(weights.shape[1]):
            partial = int(np.sum(fmap[ic] * weights[oc, ic]))
            print(f"\nWeight[filter={oc}][in_ch={ic}]:")
            print(weights[oc, ic])
            print(f"Partial sum from input channel {ic}: {partial}")

        print(f"\nLayer-2 raw output filter {oc}: {out_raw[oc]}")
...
def classifier_argmax(logits: np.ndarray) -> tuple[int, int]:
    """
    Simple classifier stage.
    ...
    if logits.shape[0] != 4:
        raise ValueError(f"Expected 4 logits, got shape {logits.shape}")

    predicted_class = int(np.argmax(logits))
    max_value = int(logits[predicted_class])

    return predicted_class, max_value


def print_classifier_debug(
    logits: np.ndarray,
    predicted_class: int,
    max_value: int
) -> None:
    print("\n" + "=" * 80)
    print("CLASSIFIER GOLDEN MODEL DEBUG")
    print("=" * 80)

    print("\nLayer-2 logits:")
    for idx, val in enumerate(logits):
        print(f"Class {idx} ({CLASS_NAMES[idx]}): {int(val)}")

    print("\nPredicted class:")
    print(f"class={predicted_class} ({CLASS_NAMES[predicted_class]})")
...
def main():
    ...
    py_l2_quant = quantize_int16(py_l2_relu)

    # ------------------------------------------------------
    # Classifier Golden Model
    # ------------------------------------------------------
    py_pred_class, py_pred_max = classifier_argmax(py_l2_quant)
    ...
    print_classifier_debug(
        py_l2_quant,
        py_pred_class,
        py_pred_max
    )

    compare_classifier_output(
        rtl_classifier_full_data,
        py_pred_class,
        py_pred_max
    )

    rtl_l2_full_vec = reconstruct_layer2_vector(
        rtl_l2_full_data,
        num_filters=4
    )

    compare_layer2_vector(
        "Full Pipeline Layer-2 Output",
        rtl_l2_full_vec,
        py_l2_quant
    )
