...
module conv1_engine_iq #(
  ...
  // 4 output channels × 2 input channels (I/Q) × 9 kernel weights
  logic signed [W_W-1:0] weight [0:3][0:1][0:8];

  // selected weights for the current output channel
  ...
  // select weights for current output channel
  always_comb begin
