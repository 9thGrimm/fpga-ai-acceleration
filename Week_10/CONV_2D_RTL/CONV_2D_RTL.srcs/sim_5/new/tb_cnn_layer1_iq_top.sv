...
module tb_cnn_layer1_iq_top;
  ...
  initial begin
    ...
    $display("quant_unknown_count    = %0d", quant_unknown_count);
  ...
  // ---------------- Conv/ReLU/Quant debug monitor ----------------
  always @(posedge clk) begin
    #1step;

    if (dut.conv_valid) begin
      $display("STAGE   t=%0t  ch=%0d  row=%0d col=%0d  conv_y=%0d (b=%b h=%h)  relu_y=%0d (b=%b h=%h)  quant_y=%0d (b=%b h=%h)",
               $time,
               dut.conv_ch,
               dut.row_idx_i,
               dut.col_idx_i,
               ...
               dut.quant_y, dut.quant_y, dut.quant_y);
      ...
      if ($isunknown(dut.quant_y)) begin
        quant_unknown_count = quant_unknown_count + 1;
        $display("  --> WARNING: quant_y has X/Z at t=%0t", $time);
      end
    end
  end
